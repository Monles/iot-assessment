    ...
    int main() {
        ...
        while (1) {
            ...
            if (uBit.buttonA.isPressed() && uBit.buttonB.isPressed()) {
                ...
                std::string encryptedText2 = encrypt(sa1, key, iv);
                std::string cipher = sa1 + encryptedText1;
                ...
                uBit.serial.printf("\r\n\r Adding salt, the cipher is sent... %s\r\n", cipher.c_str());

                // Send the encrypted message
                uBit.radio.datagram.send(const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(cipher.data())), cipher.size());
