...
int main() {
    ...
    while (1) {
        ...
        if (uBit.buttonA.isPressed() && uBit.buttonB.isPressed()) {
            ...
            // Create a text combined with command
            std::string command = "zy";
           // Use AES-ECB-128 to encrypt the text with dpk 
            std::string encryptedText1 = encrypt(command, key, iv);
            std::string originalText = encryptedText1 + sa1;
            // Use AES-ECB-256 to ecrypt the first cipher with dpk appending salt with temporary key (key1)
            std::string encryptedText2 = encrypt(originalText, key1, iv);

            // Print the original and encrypted strings
            uBit.serial.printf("\r\n command(The first 2 chars) + salt: %s\r\n", originalText.c_str());
            uBit.serial.printf("\r\n\r Encrypted Text with dpk: %s\r\n", encryptedText1.c_str());
            uBit.serial.printf("\r\n\r Encrypted Text appending salt with temporary key: %s\r\n", encryptedText2.c_str());

            // Send the encrypted message
            uBit.radio.datagram.send(const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(encryptedText2.data())), encryptedText2.size());
